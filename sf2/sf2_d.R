...
p4 <- dat[k,] %>% 
  ...
  annotation_logticks() +
  facet_grid('Genome-wide (100kb bins)' ~ .) +
  theme(plot.background = element_blank(),
        panel.background = element_rect(fill = NA, color = 'black', size = 1),
        panel.grid = element_blank(),
        panel.grid.major = element_line(color = 'grey70', linetype = 'dashed'),
...
{wrap_plots(wrap_plots(p1, p2, ncol = 1), wrap_plots(wrap_ggplot_grob(g), p4, ncol = 1), nrow = 1) &
  theme(plot.background = element_blank())} %>%
  ggsave('sf2_d.png', ., height = 5.6, width = 5.5, bg = 'transparent')
