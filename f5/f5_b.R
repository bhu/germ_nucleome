...
fread('../data/peaks/K9me3/ChromTime.bed.gz', sep = '\t', select = 1:5) %>%
  ...
  scale_color_manual(values = clrs, guide = guide_legend(nrow = 2)) +
  scale_fill_manual(values = clrs, guide = guide_legend(nrow = 2)) +
  ...
  ggsave('f5_b.pdf', p, height = 4.35, width = 3.5, device = cairo_pdf, bg = 'transparent')
