...
wrap_plots(p4, p3 + theme(axis.title.x = element_text(margin = margin(t = -20, unit = "pt"))),
  ...
  theme(plot.background = element_blank())  &
  ggsave('sf14_de.pdf', height = 4.54, width = 8.5)
  
ggsave('sf14_de_leg_bot.pdf', cowplot::get_legend(p3 + theme(legend.position = 'left', legend.justification = c(.5,.5))),height=4.54)
ggsave('sf14_de_leg_top.pdf', cowplot::get_legend(p4 + theme(legend.position = 'left', legend.justification = c(.5,.5))),height=4.54)
