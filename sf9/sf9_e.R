...
pd <- d50 %>% 
  select(chr, start, samp, PC1) %>%
  mutate(samp = rnm(samp)) %>%
  pivot_wider(names_from = 'samp', values_from = 'PC1') %>%
...
ggplot(pd, aes(x = x, y = est, color = samp, fill = samp)) + 
  ...
  scale_x_continuous(breaks = c(0, .5, 1),
                     labels = c('pter', '', 'qter'),
                     expand = expansion(0)) +
  facet_grid(.~'Distribution of compartment score across chromosomes') +
...
ggsave('sf7_e.pdf', p, width = 5.82, height = 3)
