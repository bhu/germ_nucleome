...
pd <- d %>%
  ...
  mutate(clu = c('Repressed', 'Polycomb', 'Active', 'Bivalent',
                 'CTCF', 'Readthrough', 'Enhancer')[cl]) %>%
  arrange(-cl) %>%
  mutate(clu = fct_inorder(clu)) %>%
  {.[.$gene %in% Reduce(intersect, split(.$gene, .$type)),]} %>%
  select(clu, Expression = SC3, CTCF, Rad21,  ATAC, K4me1, K4me3, K27ac,
         K27me3, Ring1b, H2Aub, K36me2, K36me3, K9me2, K9me3) %>%
  ...
  ungroup() %>%
  arrange(clu) %>%
  mutate(clu = sprintf('<span style=\'color:%s\'>%s</span>', 
                       cclrs[as.numeric(clu)], clu) %>% 
           fct_inorder(),
         mark = factor(mark, odr),
         y = as.numeric(mark)) 
ggplot(pd, aes(x = clu, y = y)) +
  geom_tile(aes(fill = v), data = ~subset(., mark=='Expression')) +
  scale_fill_gradientn(expression(log[2]~"RPM"), colors = brewer.blues(25), breaks = c(6, 7),
                       guide = guide_colorbar(barheight = 4, barwidth = .5,
  ...
  scale_fill_gradientn('Enrichment', colors = rev(brewer.brbg(25)), 
                       ...
                       guide = guide_colorbar(barheight = 4, barwidth = .5, title.hjust = .5,
                                              title.position = 'left', title.vjust = 1, order = 2)) +
...
ggsave('f3_d.pdf', p, height = 3.1, width = 6.5)
