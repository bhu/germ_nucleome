...
pl <- ggplot(vss$DE, aes(x = num, fill = stat, y = 1)) + 
  geom_tile() + 
...
pb <- ggplot(vss$DI, aes(x = num, fill = stat, y = 1)) + 
  geom_tile() + 
  scale_fill_gradient2(low = clrs[2], 
                       high = clrs[1], 
                       limits = c(-1,1), 
...
pm <- ggplot(pd, aes(x = x, y = y, fill = z)) +
  geom_tile() +
  ...
  coord_cartesian(expand = F) +
  facet_wrap(~'Cis-regulatory coordination') +
  theme(plot.background = element_blank(),
        panel.background = element_blank(),
...
{ wrap_plots(pl, pm, plot_spacer(), pb, heights = c(20, 1),
  ...
  theme(plot.background = element_blank()) } %>%
  ggsave('f3_g.pdf', ., width = 4.85, height = 4.5) 
